...
export default class Common {
  elements = {
    ...
    selectByName: (name: string) =>
      this.elements
        ...
        .should('be.visible'),
    inputByPlaceholder: (placeholder: string) => cy.get(`input[placeholder="${placeholder}"]`).should('be.visible'),
  ...
  typeInputByPlaceholder = (type: inputType, placeholder: string, text: string) => {
    const input = this.elements.inputByPlaceholder(placeholder).typeRandom(text, type);
    if (text) input.invoke('val').as(slug(placeholder));
    cy.wait(0);
  };
