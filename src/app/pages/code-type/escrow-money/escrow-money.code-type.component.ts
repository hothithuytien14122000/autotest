...
@Component({
  ...
  templateUrl: './escrow-money.code-type.component.html',
  styleUrls: ['./escrow-money.code-type.component.less'],
  providers: [GlobalFacade, CodeTypesFacade, Message, FormatCurrencyPipe],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class EscrowMoneyCodeTypeComponent implements OnInit, OnDestroy {
  constructor(
  ...
  handleBack() {
    ...
    this.codeTypesFacade.query$.pipe(take(1)).subscribe((query) => {
      this.router.navigate(['/code-types'], {
        relativeTo: this.route,
        queryParams: query || {},
        queryParamsHandling: 'merge',
      });
