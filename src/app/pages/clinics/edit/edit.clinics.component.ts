...
import { AddressFacade, AttachmentTemplate, UploadFacade, ClinicsFacade, GlobalFacade } from '@store';
...
@Component({
  ...
  providers: [ClinicsFacade, AddressFacade, UploadFacade],
})
  ...
  handleSubmit(validateForm: FormGroup) {
    ...
    if (id) {
      ...
      this.clinicsFacade.status$.pipe(takeUntil(this.destroyed$)).subscribe((status) => {
        if (status == 'putOk') {
          this.router.navigate([`/clinics/detail/${this.route.snapshot.params.id}`]);
        }
      });
    }
  }
